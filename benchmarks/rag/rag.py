...
def parse_arguments():
    ...
    parser.add_argument(
        "--prompt-build-method", type=str, required=True, help="Prompt build method"
    )
    parser.add_argument(
        "--base-url",
        type=str,
    ...
    parser.add_argument(
        "--time", type=int, default=None, help="The total running time in seconds"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Whether to enable verbose logging"
    )
    parser.add_argument(
        "--max-tokens", type=int, default=32, help="Max tokens for each generation"
    )
    parser.add_argument(
    ...
    def summary(self, start_time: float, end_time: float) -> pd.DataFrame:
        ...
        for i in range(cnt):
            if self._build_method == PromptBuildMethodType.QA:
                ...
                raise ValueError("Invalid prompt build " f"method {self._build_method}")
