...
def dumb_cfg():
    return LMCacheEngineConfig.from_defaults(
        local_device="cuda", remote_url=None, remote_serde=None, enable_blending=True
    )
...
def test_spt_hit_miss(fmt, autorelease):
    ...
    for flag, token_ids_tensor, kv in zip(has_insterted, token_ids_tensors, kvs):
        if flag:
            engine.store(token_ids_tensor, kv)
...
def test_spt_partial_hit(fmt, autorelease):
    ...
    for ilen, token_ids_tensor, kv in zip(inserted_length, token_ids_tensors, kvs):
...
def test_spt_multi_query(fmt, autorelease):
    ...
    def check_groups(*ids):
        ...
        for layer_id in range(32):
            ...
            assert (
                result2.valid_mask[0:target_len2] == 1
            ).all(), "Should be all valid!"
            assert (
                result2.valid_mask[target_len2:] == 0
...
def test_cacheblend_executor_single_query():
    ...
    assert torch.equal(
        ret.local_indices, torch.arange(prefix_len, dtype=torch.int, device="cpu")
    )
