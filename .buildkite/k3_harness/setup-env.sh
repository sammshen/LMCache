...
echo "--- :wrench: Install sitecustomize.py to pre-import transformers"
# Context: vLLM nightly PR vllm-project/vllm#40331 introduced a daemon
# thread at module-scope of vllm/entrypoints/cli/main.py that calls
# `import transformers` in parallel with the main thread. On the K3s
# pods, that thread races the main thread's later
#   vllm/transformers_utils/config.py:18
#   from transformers import GenerationConfig, PretrainedConfig
# and the main thread deterministically observes a half-initialized
# transformers._LazyModule (registered in sys.modules, _class_to_module
# not yet populated). The result is:
#   ImportError: cannot import name 'GenerationConfig' from 'transformers'
# Build #2653's diagnostic dump confirmed the install itself is correct
# and an isolated `from transformers import GenerationConfig` works;
# only vllm's threaded import path hits the window. Local venvs (warm
# disk) finish `import transformers` fast enough to close the window
# before the main thread gets there -- the pods (cold overlayfs) do not.
#
# Defuse the race by pre-importing transformers on the main thread via
# site.py. Python runs sitecustomize.py during interpreter startup,
# before any user code in the target script (including
# /opt/venv/bin/vllm). Once transformers is fully in sys.modules, any
# later `import transformers` -- including whatever preload form vllm's
# current nightly uses -- is a no-op against a fully-wired _LazyModule,
# and attribute lookups for GenerationConfig / PretrainedConfig succeed.
#
# This is deliberately independent of vllm's source layout. An earlier
# iteration of this fix (reverted) patched vllm/entrypoints/cli/main.py
# directly with a specific text needle, which broke the moment upstream
# restructured the file. sitecustomize sidesteps vllm entirely -- it
...
# Pre-imports transformers on the main thread to defuse the race
# introduced by vllm's background-thread import preload. Once upstream
# vllm stops running that preload, this file can be removed with no
# functional change -- `import transformers` is idempotent.
try:
    import transformers  # noqa: F401
except Exception:
    # sitecustomize must never crash the interpreter. If the
    # transformers install is broken, let the real import site fail
    # with its own specific traceback rather than a startup error.
    pass
PY
python -c "import sitecustomize, transformers; print(f'sitecustomize active; transformers {transformers.__version__} pre-imported')"
