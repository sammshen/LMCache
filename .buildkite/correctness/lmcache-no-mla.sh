...
# Generalized LMCache without MLA script
# Usage: ./lmcache-no-mla.sh <model> <output_file> [max_model_len]
# Example: ./lmcache-no-mla.sh "deepseek-ai/DeepSeek-V2-Lite" "lmcache_no_mla.txt" 6000
# Example: ./lmcache-no-mla.sh "meta-llama/Meta-Llama-3.1-8B-Instruct" "lmcache_dense_no_mla.txt" 12000

MODEL=${1:-"deepseek-ai/DeepSeek-V2-Lite"}
OUTPUT_FILE=${2:-"lmcache_no_mla.txt"}
MAX_MODEL_LEN=${3:-6000}
VLLM_MLA_DISABLE=1  # Always disable MLA for this script

export PORT=8000

echo "🚀 Starting LMCache (no MLA) test with:"
echo "   Model: $MODEL"
...
echo "✅ LMCache (no MLA) test completed for $MODEL"
