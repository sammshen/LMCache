...
class LMCacheEngineConfig:
    ...
    def from_env() -> "LMCacheEngineConfig":
        ...
        config.local_disk = parse_env(get_env_name("local_disk"), config.local_disk)
        ...
        config.blend_recompute_ratio = to_float(
            parse_env(
                get_env_name("blend_recompute_ratio"), config.blend_recompute_ratio
            )
        )
    ...
    def validate(self) -> "LMCacheEngineConfig":
        ...
        if self.enable_nixl:
            ...
            assert (
                self.save_decode_cache is False
