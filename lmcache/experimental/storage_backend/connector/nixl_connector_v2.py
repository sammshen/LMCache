...
class NixlPipe:
    ...
    def __init__(
        self, nixl_config: NixlConfig, side_channel: zmq.Socket
    ):  # type: ignore
        ...
        if nixl_config.buffer_size > NixlPipe.TRANSFER_BUFFER_SIZE:
            assert nixl_config.buffer_size % NixlPipe.TRANSFER_BUFFER_SIZE == 0, (
                f"Buffer size must be a multiple of " f"{NixlPipe.TRANSFER_BUFFER_SIZE}"
            )

        self._buffer = torch.empty(
            nixl_config.buffer_size, device=nixl_config.buffer_device, dtype=torch.uint8
        )
    ...
    def _commit_write(self, write_size: int, uid: str):
        ...
        while (status := self._agent.check_xfer_state(handle)) != "DONE":
            if status == "PROC":
                ...
                logger.error(
                    "Transfer failed with status: %s, handle: %s", status, handle
                )
                raise RuntimeError(
    ...
    def read_buffer(self, metadatas: list[MemoryObjMetadata]) -> list[MemoryObj]:
        ...
        for metadata in metadatas:
            ...
            if offset + obj_size > self.nixl_config.buffer_size:
                break
            obj = TensorMemoryObj(self._buffer[offset : offset + obj_size], metadata)
            ret.append(obj)
            offset += obj_size
...
class NixlChannel:
    ...
    def __init__(self, nixl_config: NixlConfig):
        ...
        if nixl_config.role == NixlRole.SENDER:
            self._side_channel.connect(
                "tcp://{}:{}".format(nixl_config.peer_host_name, nixl_config.peer_port)
            )
    ...
    def _receiver_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.debug("Received request with %d keys", len(request.keys))

                self._process_receive_transaction(
                    keys=request.keys, metadatas=request.metadatas
                )

            except zmq.Again as e:  # type: ignore
                # Handle the timeout when waiting for a message
                logger.debug(
                    "Timeout waiting for a message on the side channel: %s", str(e)
                )
                continue
    ...
    def finish_send(self):
        ...
        assert (
            self._added_payload_count == self._prepared_count
