...
def CreateStorageBackend(
    ...
    match config:
        case LMCacheEngineConfig(
            _, local_device=None, remote_url=str(p)
        ) if p is not None:
            # remote only
        ...
        case LMCacheEngineConfig(
            _, local_device=str(p), remote_url=None
        ) if p is not None:
            # local only
            match config.local_device:
                case "cpu" | "cuda":
                    logger.info(
                        f"Initializing local-only ({config.local_device})" f" backend"
                    )
        ...
        case LMCacheEngineConfig(
            _, local_device=str(p), remote_url=str(q)
