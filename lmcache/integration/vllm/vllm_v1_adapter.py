...
class LMCacheLookupClient:
    ...
    def __init__(self, role: KVConnectorRole, is_tp: bool, vllm_config: "VllmConfig"):
        ...
        self.socket = make_zmq_socket(
            self.ctx, socket_path, zmq.REQ, bind=False  # type: ignore[attr-defined]
        )
...
class LMCacheLookupServer:
    ...
    def __init__(
        ...
        socket_path = get_zmq_rpc_path_lmcache(role, is_tp, vllm_config)
        self.socket = make_zmq_socket(
            self.ctx, socket_path, zmq.REP, bind=True  # type: ignore[attr-defined]
        )
...
class ReqMeta:
    ...
    def from_request_tracker(
        ...
        chunk_boundary = (
            cdiv(tracker.num_saved_tokens, lmcache_chunk_size) * lmcache_chunk_size
        )
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        for request in metadata.requests:
            ...
            ret_token_mask = self.lmcache_engine.retrieve(
                tokens, token_mask, kvcaches=kvcaches, slot_mapping=slot_mapping
            )
    ...
    def wait_for_save(self):
        ...
        for request in connector_metadata.requests:
            ...
            skip_leading_tokens = max(
                self.lmcache_engine.lookup(token_ids), save_spec.skip_leading_tokens
            )
